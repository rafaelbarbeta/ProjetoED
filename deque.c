...
int startDeque (deque *d, unsigned int size) {
    ...
    if (d->vect == NULL) //significa que nao ha mais memoria no sistema. Retorna 0 para o usuario para indicar falha
        return 0;
    return 1;
}
...
int addStartDeque (deque *d, type num) {
    if (!fullDeque(d)) { //verifica se o deque nao está cheio
...
int fullDeque (deque *d){
    ...
    //retroce uma posicao do index temporario e verifica se head e tail sao iguais. Se forem, entao o deque está cheio
    nextDeque(d,&indexTemp,-1);
    if (indexTemp == d->head)
        return 1;
    else
